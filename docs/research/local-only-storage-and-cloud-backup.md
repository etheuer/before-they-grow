...
**Verified.** iCloud Backup periodically includes app data; Apple provides a per-resource exclusion mechanism. Current Apple guidance says the exclusion resource value is guidance to the system, **not a guarantee** that an item never appears in a backup or on a restored device. Android Auto Backup includes most app internal files, databases, preferences, and app-specific external files by default, and uploads backup data to the user’s Google Drive. [Apple QA1719](https://developer.apple.com/library/archive/qa/qa1719/_index.html) · [Apple backup optimization guidance](https://developer.apple.com/documentation/foundation/optimizing-your-app-s-data-for-icloud-backup) · [Android Auto Backup](https://developer.android.com/identity/data/autobackup)

**Recommendation.** Product copy may say: *“Memories are stored locally in this app. For the signed version we tested, our backup-exclusion drill observed that the test memories did not return through the tested iCloud and Google cloud-backup paths. We do not provide cloud sync or recovery.”* It must say that deleting the app can remove local memories and must not promise survival of device replacement, transfer, offload, or reinstall. Do not say memories can **never** appear in a backup or restore. A drill is evidence for one signed build and tested path, not a perpetual guarantee.

## iOS

### Storage and exclusion

**Verified.** Apple’s supported iOS 5.1+ mechanism is the `NSURLIsExcludedFromBackupKey` resource value. It is applied to the file or directory URL rather than being an app-wide switch; current Apple guidance calls it guidance rather than a never-backup/never-restore guarantee. QA1719 distinguishes this from using cache space merely to evade backup. [Apple QA1719](https://developer.apple.com/library/archive/qa/qa1719/_index.html) · [Apple `URLResourceKey.isExcludedFromBackupKey`](https://developer.apple.com/documentation/foundation/urlresourcekey/isexcludedfrombackupkey) · [Apple backup optimization guidance](https://developer.apple.com/documentation/foundation/optimizing-your-app-s-data-for-icloud-backup)
...
On **every final save or move**, apply and immediately re-verify `NSURLIsExcludedFromBackupKey = true` for **every concrete family-memory resource**: the final audio file, the metadata database and its companion files, attachments, and any generated derivative. Apple warns that common file operations can reset the value, so setting it once on a directory is insufficient release evidence. Keep a single inventory of all paths that can contain a child name, transcript, prompt-answer relation, or audio bytes; a new storage path is not releasable until it is added to this exclusion check. [Apple `URLResourceKey.isExcludedFromBackupKey`](https://developer.apple.com/documentation/foundation/urlresourcekey/isexcludedfrombackupkey) · [Apple backup optimization guidance](https://developer.apple.com/documentation/foundation/optimizing-your-app-s-data-for-icloud-backup)

### Unresolved Apple guidance tension

**Verified tension.** Apple’s current backup guidance treats user-created/nonpurgeable content differently from recreatable cache data and says arbitrary user files should not be excluded when the user cannot readily recreate them. Family recordings and reviewed transcripts are irreplaceable user content, so excluding them for a local-only privacy promise conflicts with that guidance rather than silently satisfying it. [Apple backup optimization guidance](https://developer.apple.com/documentation/foundation/optimizing-your-app-s-data-for-icloud-backup)

**Human decision required.** Product, legal/privacy, and App Review owners must explicitly accept or change this local-only durability trade-off before submission. This note does not resolve it; the backup flag and device drill do not make the tension disappear.
...
**Verified.** App-private internal storage needs no storage permission and is inaccessible to other apps; Android removes app-specific files when the app is uninstalled. Android Auto Backup lists `getNoBackupFilesDir()` among locations excluded from Auto Backup, and `Context.getNoBackupFilesDir()` defines it as excluded from **automatic backup to remote storage**. Ordinary internal files and databases are included by default. [Android app-specific storage](https://developer.android.com/training/data-storage/app-specific) · [Android Auto Backup](https://developer.android.com/identity/data/autobackup) · [Android `Context.getNoBackupFilesDir()`](https://developer.android.com/reference/android/content/Context#getNoBackupFilesDir())

**Recommendation.** Make a dedicated app-private no-backup root the canonical home for final recordings and the metadata database; do not put family memories in shared/external media storage. Put temporary capture only in cache, then atomically move it to the canonical root before committing its metadata. The metadata adapter must use that same root or be explicitly covered by the rules below—using a database library’s default location without checking it is not acceptable. The no-backup location is a remote-automatic-backup control, not a device-transfer promise. [Android Auto Backup](https://developer.android.com/identity/data/autobackup) · [Android `Context.getNoBackupFilesDir()`](https://developer.android.com/reference/android/content/Context#getNoBackupFilesDir())
...
**Recommendation.** Do not create a product transfer feature or copy that says memories migrate. Leave OS transfer behavior unpromised, record the result of the device drill, and re-review it for every target-SDK, storage-path, and backup-rule change. `getNoBackupFilesDir()` is documented for automatic remote backup; the actual transfer outcome remains a release drill rather than a marketing claim. [Android Auto Backup](https://developer.android.com/identity/data/autobackup) · [Android `Context.getNoBackupFilesDir()`](https://developer.android.com/reference/android/content/Context#getNoBackupFilesDir())
...
| iOS — iCloud Backup | Project-defined observation: enable iCloud Backup on a dedicated test account, finish a backup after saving sentinels, then use only OS backup recovery on a clean physical test device. This is a test operation, not a product restore feature or a QA1719-certified procedure. | For this signed build and tested path, no sentinel recording or metadata returns. A sentinel is a release blocker; inspect every unexpected path and repeat after correction. This observation does not create a perpetual guarantee. [Apple QA1719](https://developer.apple.com/library/archive/qa/qa1719/_index.html) · [Apple backup optimization guidance](https://developer.apple.com/documentation/foundation/optimizing-your-app-s-data-for-icloud-backup) |
...
**Gate:** do not make an absolute iCloud/Google cloud-backup claim. A matching physical cloud test may support a narrow, build-specific statement that the signed artifact had a no-sentinel result on the tested path. Do not claim device migration, offload retention, reinstall recovery, or deletion persistence at all.

**Key unresolved fact:** the behavior of **iOS OS-assisted device transfer and Offload App** for files marked with `NSURLIsExcludedFromBackupKey` is not established by the approved source set. It requires the iOS device drill and remains outside the product promise regardless of the observed result. The separate unresolved decision is whether excluding irreplaceable family content is acceptable in light of Apple’s backup guidance.
...
- [Apple — `URLResourceKey.isExcludedFromBackupKey`](https://developer.apple.com/documentation/foundation/urlresourcekey/isexcludedfrombackupkey)
- [Android Developers — *Back up user data with Auto Backup*](https://developer.android.com/identity/data/autobackup)
- [Android Developers — `Context.getNoBackupFilesDir()`](https://developer.android.com/reference/android/content/Context#getNoBackupFilesDir())
